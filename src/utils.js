...
export const formatDate = (dateStr, needHMS = true) => {
    //Mon Jan 19 1970 00:42:05 GMT+0400 (GMT+04:00)
    const date = new Date(Number(dateStr) * 1000)

    return `${date.getDate()}/${date.getMonth() + 1}/${date.getUTCFullYear()}${needHMS ? ` ${pad(date.getHours())}:${pad(date.getMinutes())}:${pad(date.getSeconds())}` : ''}`;
}
